...
test('baseline: transform stream', function (t) {
  ...
  var s = select('li', function (e) {
    var tr = e.createStream({ inner: true })
...
test('read (outer) to write (outer)', function (t) {
  ...
  var s = select('li', function (e) {
    var rs = e.createReadStream()
    var ws = e.createWriteStream()
...
test('read inner to write inner', function (t) {
  ...
  var s = select('li', function (e) {
    var rs = e.createReadStream({ inner: true })
    var ws = e.createWriteStream({ inner: true })
    rs.pipe(through.obj()).pipe(ws)
  })
  readStream()
    .pipe(tokenize())
